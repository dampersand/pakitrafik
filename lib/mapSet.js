...
const mapSet = {
	...
	//delete a map from the mapset.  Synchronous, no need for a promise.
	deleteMap(index) {
	...
	initMapSet() {
		...
		return mapFactory.listMaps().then(function(mapList){
			...
			for (index in mapList) {
				promises.push(mapFactory.newMapFromFile(mapList[index]).then(function(loadedMap){addMap(loadedMap)}));
			}
